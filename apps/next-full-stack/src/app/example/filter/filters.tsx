...
export function AnimateChangeInHeight({ children, className }: { children: React.ReactNode; className?: string }) {
...
const FilterIcon = ({ type }: { type: FilterType | Status | Assignee | Labels | Priority }) => {
  switch (type) {
...
export const assigneeFilterOptions: FilterOption[] = Object.values(Assignee).map((assignee) => ({
  name: assignee,
...
export const priorityFilterOptions: FilterOption[] = Object.values(Priority).map((priority) => ({
  name: priority,
...
const filterOperators = ({ filterType, filterValues }: { filterType: FilterType; filterValues: string[] }) => {
  switch (filterType) {
    case FilterType.STATUS:
    case FilterType.ASSIGNEE:
    case FilterType.PRIORITY:
