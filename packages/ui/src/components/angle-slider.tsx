...
function hasMinStepsBetweenValues(values: number[], minStepsBetweenValues: number) {
  if (minStepsBetweenValues > 0) {
    ...
    const actualMinStepsBetweenValues = stepsBetweenValues.length > 0 ? Math.min(...stepsBetweenValues) : 0;
...
function AngleSlider(props: AngleSliderProps) {
  ...
  }, [store, min, max, step, minStepsBetweenThumbs, size, thickness, startAngle, endAngle, disabled, inverted]);
...
function AngleSliderRange(props: React.ComponentProps<'path'>) {
  ...
  const rangeEnd = values.length <= 1 ? (sortedValues[0] ?? min) : (sortedValues[sortedValues.length - 1] ?? max);
...
function AngleSliderValue(props: AngleSliderValueProps) {
  ...
  return (
    <ValuePrimitive
      ...
      {...valueProps}
      className={cn(
        'text-foreground pointer-events-none flex items-center justify-center text-sm font-medium select-none',
        className,
      )}
      style={{
        ...valueStyle,
