...
function CheckboxGroup({ className, ...props }: React.ComponentProps<typeof CheckboxGroupPrimitive.Root>) {
  return (
...
function CheckboxGroupLabel({ className, ...props }: React.ComponentProps<typeof CheckboxGroupPrimitive.Label>) {
  return (
...
function CheckboxGroupList({ className, ...props }: React.ComponentProps<typeof CheckboxGroupPrimitive.List>) {
  return (
...
function CheckboxGroupItem({
  ...
  return (
    <label className="has-data-invalid:text-destructive flex w-fit items-center gap-2 text-sm leading-none select-none has-data-disabled:cursor-not-allowed has-data-disabled:opacity-50">
      <CheckboxGroupPrimitive.Item
        data-slot="checkbox-group-item"
        className={cn(
          'border-primary focus-visible:ring-ring data-invalid:border-destructive [&[data-state=checked]:not([data-invalid])]:bg-primary [&[data-state=checked]:not([data-invalid])]:text-primary-foreground [&[data-state=checked][data-invalid]]:bg-destructive [&[data-state=checked][data-invalid]]:text-primary-foreground size-4 shrink-0 rounded-sm border shadow-sm focus-visible:ring-1 focus-visible:outline-hidden [&[data-state=unchecked][data-invalid]]:bg-transparent',
          className,
        )}
        {...props}
      >
...
function CheckboxGroupDescription({
  ...
  return (
    <CheckboxGroupPrimitive.Description
      ...
      className={cn('text-muted-foreground data-invalid:text-destructive text-[0.8rem] leading-none', className)}
      {...props}
    />
  );
}

function CheckboxGroupMessage({ className, ...props }: React.ComponentProps<typeof CheckboxGroupPrimitive.Message>) {
  return (
