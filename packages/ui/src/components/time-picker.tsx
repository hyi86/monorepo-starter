...
import { Popover, PopoverAnchor, PopoverContent, PopoverTrigger } from '@monorepo-starter/ui/components/popover';
import { VisuallyHiddenInput } from '@monorepo-starter/ui/components/visually-hidden-input';
import { useAsRef } from '@monorepo-starter/ui/hooks/use-as-ref';
import { useIsomorphicLayoutEffect } from '@monorepo-starter/ui/hooks/use-isomorphic-layout-effect';
import { useLazyRef } from '@monorepo-starter/ui/hooks/use-lazy-ref';
...
function focusFirst(candidates: React.RefObject<ColumnItemElement | null>[], preventScroll = false) {
...
function TimePicker(props: TimePickerProps) {
  ...
  const onPopoverOpenChange = React.useCallback((newOpen: boolean) => store.setState('open', newOpen), [store]);
...
const TimePickerInputGroupContext = React.createContext<TimePickerInputGroupContextValue | null>(null);
...
function TimePickerInputGroup(props: DivProps) {
  ...
  const onInputRegister = React.useCallback((segment: Segment, ref: React.RefObject<InputElement | null>) => {
  ...
  const getNextInput = React.useCallback((currentSegment: Segment): React.RefObject<InputElement | null> | null => {
    ...
    (event: React.MouseEvent<InputGroupElement>) => {
      ...
      if (inputGroupClickAction === 'open') {
        ...
        const isInputAlreadyFocused =
          activeElement && activeElement.tagName === 'INPUT' && inputGroupRef.current?.contains(activeElement);
    ...
    <TimePickerInputGroupContext.Provider value={inputGroupContextValue}>
      <PopoverAnchor>
        <InputGroupPrimitive
          ...
          {...inputGroupProps}
          className={cn(
            'border-input bg-background flex h-10 w-full cursor-text items-center gap-0.5 rounded-md border px-3 py-2 shadow-xs transition-shadow outline-none',
    ...
    (event: React.KeyboardEvent<InputElement>) => {
      ...
      if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
        ...
        if (inputGroup && inputRef.current) {
          const allInputs = Array.from(inputGroup.querySelectorAll('input[type="text"]')) as HTMLInputElement[];
...
function TimePickerTrigger(props: ButtonProps) {
  ...
  return (
    <PopoverTrigger
      ...
      {...triggerProps}
      className={cn(
        "text-muted-foreground hover:text-foreground ml-auto flex items-center transition-colors disabled:pointer-events-none [&>svg:not([class*='size-'])]:size-4",
        className,
      )}
    >
      {children ?? <Clock />}
...
function TimePickerContent(props: TimePickerContentProps) {
  ...
  const onInteractOutside: NonNullable<PopoverContentProps['onInteractOutside']> = React.useCallback(
    (event) => {
...
interface TimePickerColumnContextValue {
  getItems: () => ItemData[];
  onItemRegister: (value: number | string, ref: React.RefObject<ColumnItemElement | null>, selected: boolean) => void;
...
function TimePickerColumnItem(props: TimePickerColumnItemProps) {
  ...
  const formattedValue =
    typeof value === 'number' && format === '2-digit' ? value.toString().padStart(2, '0') : value.toString();
...
function TimePickerHour(props: TimePickerHourProps) {
  ...
  return (
    <HourPrimitive
      ...
      {...hourProps}
      className={cn('scrollbar-none flex max-h-[200px] flex-col gap-1 overflow-y-auto p-1', className)}
    >
      {hours.map((hour) => (
