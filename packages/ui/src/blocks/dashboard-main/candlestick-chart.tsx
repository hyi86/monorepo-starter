...
import { ChartConfig, ChartContainer, ChartTooltip } from '@monorepo-starter/ui/components/chart';
import { Bar, BarChart, CartesianGrid, Cell, ReferenceLine, TooltipProps, XAxis, YAxis } from 'recharts';
    ...
    return (
      <div className="bg-popover text-popover-foreground grid min-w-32 items-start gap-1.5 rounded-lg border px-3 py-1.5 text-xs">
        ...
        <p className="text-muted-foreground">
          Open: <span className="text-foreground font-medium">{data.openClose[0]}</span>
        </p>
        <p className="text-muted-foreground">
        ...
        <p className="text-muted-foreground">
          Close: <span className="text-foreground font-medium">{data.openClose[1]}</span>
        </p>
      </div>
    );
  }
...
function CandlestickChart() {
  ...
  return (
    <div className="w-full rounded-lg border">
      <ChartContainer
        config={chartConfig}
        className="[&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-border/64 [&_.recharts-cartesian-axis-line]:stroke-border/64 [&_.recharts-cartesian-axis-tick_text]:fill-muted-foreground/72 dark:[&_.recharts-cartesian-axis-tick_text]:fill-muted-foreground/64 aspect-auto h-110 w-full [&_.recharts-rectangle.recharts-tooltip-cursor]:fill-zinc-950/5 dark:[&_.recharts-rectangle.recharts-tooltip-cursor]:fill-zinc-950/25"
      >
              ...
              label={({ viewBox }) => (
                <g transform={`translate(${viewBox.x + viewBox.width + 5},${viewBox.y})`}>
                  <rect
                    ...
                    rx={4}
                  />
                  <text x={2} y={4} fill="#fff" fontSize={12} fontWeight="500" textAnchor="start">
