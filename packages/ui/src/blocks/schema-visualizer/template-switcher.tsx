  ...
  return (
    <Breadcrumb>
      <BreadcrumbList>
        ...
        <BreadcrumbItem>
          <DropdownMenu>
            <DropdownMenuTrigger className="text-foreground flex items-center gap-1 font-medium">
              {activeTemplate?.name ?? 'Select Template'}
              <ChevronDownIcon className="-me-1 opacity-60" size={16} aria-hidden="true" />
            </DropdownMenuTrigger>
            <DropdownMenuContent sideOffset={10}>
              {templates.map((template) => (
                <DropdownMenuItem key={template.name} onClick={() => setActiveTemplate(template)}>
                  {template.name}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
