...
interface SpreadsheetProps<TRow extends SpreadsheetRow, TValue = unknown> extends React.HTMLAttributes<HTMLDivElement> {
...
function SpreadsheetInner<TRow extends SpreadsheetRow, TValue = unknown>(
  ...
  const columnMeta = useMemo<ColumnInfo[]>(() => {
    return table.getAllLeafColumns().map((column) => ({
      id: column.id,
      accessorKey:
        typeof (column.columnDef as any).accessorKey === 'string' ? (column.columnDef as any).accessorKey : column.id,
    }));
  }, [table]);
...
export const Spreadsheet = React.forwardRef(SpreadsheetInner) as <TRow extends SpreadsheetRow, TValue = unknown>(
