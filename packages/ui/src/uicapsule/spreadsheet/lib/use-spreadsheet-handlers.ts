...
export const useSpreadsheetHandlers = ({ columns, navigationMap }: UseSpreadsheetHandlersProps) => {
  ...
  const handleMouseDown = useCallback(
    ...
    [selectedCells, columns, data, editingCell, setSelectedCells, setEditingCell, setIsDragging, setDragStartCell],
  );
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent, rowId: string, columnId: string) => {
      ...
      const rangeCells = getRangeCells(dragStartCell.rowId, dragStartCell.columnId, rowId, columnId, columns, data);
      const newSelection = new Set(rangeCells);
      setSelectedCells(newSelection);
