...
export interface SpreadsheetStore<TRow extends SpreadsheetRow = SpreadsheetRow> {
  ...
  updateSelectedCellsData: (
    value: unknown | ((options: { row: SpreadsheetRow; columnId: string; currentValue: unknown }) => unknown),
  ) => void;
        ...
        if (rowIndex >= 0 && rowIndex < newData.length) {
          ...
          const newValue =
            typeof value === 'function'
              ? (value as (options: { row: SpreadsheetRow; columnId: string; currentValue: unknown }) => unknown)({
                  row: currentRow,
                  columnId,
