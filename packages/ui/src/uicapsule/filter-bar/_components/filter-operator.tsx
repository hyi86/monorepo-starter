...
import type { Column, ColumnDataType, DataTableFilterActions, FilterModel, FilterOperators } from '../filter-package';
import {
        ...
        render={
          <Button
            ...
            onClick={(e) => {
              ...
              e.preventDefault();
              const opDetails = filterTypeOperatorDetails.boolean[filter.operator as FilterOperators['boolean']];

              actions.setFilterOperator(column.id, opDetails.isNegated ? opDetails.negationOf : opDetails.negation);
            }}
          />
        }
      >
      ...
      <PopoverContent align="start" className="w-fit origin-(--radix-popover-content-transform-origin) p-0">
        <Command loop>
          ...
          <CommandList className="max-h-fit">
            <FilterOperatorController filter={filter} column={column} actions={actions} closeController={close} />
          </CommandList>
        </Command>
      </PopoverContent>
    </Popover>
...
function FilterOperatorBooleanController<TData>({
  ...
  const relatedFilters = Object.values(booleanFilterOperators).filter((o) => o.target === filterDetails.target);
