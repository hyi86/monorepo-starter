...
function __FilterValue<TData, TType extends ColumnDataType>({
  ...
  return (
    <Popover>
      ...
      >
        <FilterValueDisplay filter={filter} column={column} actions={actions} entityName={entityName} />
      </PopoverTrigger>
      <PopoverContent
        align="start"
        side="bottom"
      ...
      >
        <FilterValueController filter={filter} column={column} actions={actions} strategy={strategy} />
      </PopoverContent>
    </Popover>
  );
}
...
export function FilterValueOptionDisplay<TData>({ filter, column, actions }: FilterValueDisplayProps<TData, 'option'>) {
...
export function FilterValueDateDisplay<TData>({ filter, column, actions }: FilterValueDisplayProps<TData, 'date'>) {
...
export function FilterValueTextDisplay<TData>({ filter, column, actions }: FilterValueDisplayProps<TData, 'text'>) {
  if (!filter) return null;
...
export function FilterValueNumberDisplay<TData>({ filter, column, actions }: FilterValueDisplayProps<TData, 'number'>) {
...
export function FilterValueBooleanDisplay<TData>({ filter, column }: FilterValueDisplayProps<TData, 'boolean'>) {
...
const OptionItem = memo(function OptionItem({ option, onToggle }: OptionItemProps) {
  ...
  return (
    <CommandItem key={value} onSelect={handleSelect} className="group flex items-center justify-between gap-4">
  ...
  return (
    <Command className="max-w-[300px]" loop>
      ...
      <CommandList>
        ...
        <CommandSeparator
          className={cn((unselectedOptions.length === 0 || selectedOptions.length === 0) && 'hidden')}
        />
  ...
  return (
    <Command>
      <CommandList className="max-h-fit">
        <CommandGroup>
          <CommandItem>
            <DebouncedInput placeholder="search" autoFocus value={filter?.values[0] ?? ''} onChange={changeText} />
          </CommandItem>
        </CommandGroup>
      </CommandList>
    </Command>
