...
export type FiltersStateUpdaterFn<TContext> = (prev: FiltersState, next: FiltersState, context?: TContext) => void;
 ...
 */
export type TTransformValueToOptionFn<TVal = unknown> = (value: ElementType<NonNullable<TVal>>) => ColumnOption;
...
export type TBuiltInOrderFn = (a: ColumnOption, b: ColumnOption, direction: OrderDirection) => number;
...
export type ColumnConfig<TData, TType extends ColumnDataType = any, TVal = unknown, TId extends string = string> = {
  ...
  transformValueToOptionFn?: TType extends OptionBasedColumnDataType ? TTransformValueToOptionFn<TVal> : never;
...
export interface DataTableFilterActions<TContext = any> {
  ...
  batch: (callback: (batchActions: DataTableFilterBatchActions) => void, context?: TContext) => void;
}
 ...
 *
 */
export type FilterOperatorDetails<OperatorValue, T extends ColumnDataType> = FilterOperatorDetailsBase<
  OperatorValue,
  T
> &
  (
...
> {
  ...
  filters?: FiltersState;
  onFiltersChange?: React.Dispatch<React.SetStateAction<FiltersState>> | FiltersStateUpdaterFn<TContext>;
