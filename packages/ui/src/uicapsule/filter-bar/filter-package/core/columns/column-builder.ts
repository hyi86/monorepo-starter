...
> {
  ...
  toggledStateName(value: string): ColumnConfigBuilder<TData, TType extends 'boolean' ? TType : never, TVal, TId> {
    if (this.config.type !== 'boolean') throw new Error('toggledStateName() is only applicable to boolean columns');
  ...
  private validateType(expectedTypes: ColumnDataType | ColumnDataType[], methodName: string) {
    ...
    if (!isAnyOf(this.config.type, types)) {
      throw new Error(`[Column config builder] ${methodName} is only applicable to ${types.join(' or ')} columns`);
    }
  }
