...
export class ColumnDataService<TData> {
   ...
   */
  computeBaseOptions<TType extends ColumnDataType, TVal>(column: ColumnConfig<TData, TType, TVal>): ColumnOption[] {
    ...
    if (column.transformValueToOptionFn) {
      columnOptions = uniqueValues.map((m) => column.transformValueToOptionFn!(m as ElementType<NonNullable<TVal>>));
    }
   ...
   */
  computeOrderedOptions<TType extends ColumnDataType, TVal>(column: ColumnConfig<TData, TType, TVal>): ColumnOption[] {
  ...
  getValues<TType extends ColumnDataType, TVal>(
    ...
    if (column.transformValueToOptionFn) {
      const memoizedTransform = memo(
        () => [raw],
        (deps) => (deps[0] ?? []).map((v) => column.transformValueToOptionFn!(v) as ElementType<NonNullable<TVal>>),
  ...
  computeFacetedMinMaxValues<TType extends ColumnDataType, TVal>(
    ...
    const rawValues = this.data
      ...
      .filter((v): v is NumericValue => (typeof v === 'number' && !Number.isNaN(v)) || typeof v === 'bigint');
