 ...
 * ```
 */
export function isBuiltInOrderFnTuple(value: unknown): value is [TBuiltInOrderFnName, OrderDirection] {
  return Array.isArray(value) && value.length === 2 && isBuiltInOrderFnName(value[0]) && isOrderDirection(value[1]);
}
